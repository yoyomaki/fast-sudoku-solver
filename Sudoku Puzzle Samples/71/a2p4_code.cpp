...
void generate_children2(vector<vector<int> >& puzzle, map<pair<int,int>, unordered_set<int> >& unassigned_pos, cell *parent, vector<cell*> &children){
    int min = INT_MAX;
    int r;
    int c;
    int tie;
...
void unassigned_insert(map<pair<int,int>, unordered_set<int> >& unassigned_pos, int r, int c, int value){
    pair<int, int> p;
    for(map<pair<int,int>, unordered_set<int> >::iterator it = unassigned_pos.begin(); it != unassigned_pos.end(); ++it){
...
//go back to the first valid node
int back_tracking(cell *current, vector<vector<int> >& puzzle, map<pair<int,int>, unordered_set<int> >& unassigned_pos){
    int result;
    cell *parent = current->parent;
    cell *node = current;
    while(parent->children.size() == 0){
        puzzle[node->r][node->c] = 0;
        result += 1;
        unassigned_insert(unassigned_pos, node->r, node->c, node->value);
        cell * tmp = parent;
...
//false, otherwise
bool forward_check(map<pair<int,int>, unordered_set<int> >& unassigned_pos){
    for(map<pair<int,int>, unordered_set<int> >::iterator it = unassigned_pos.begin(); it != unassigned_pos.end(); ++it){
        if(it->second.size() == 0){
            return true;
        }
...
void unassigned_update(map<pair<int,int>, unordered_set<int> >& unassigned_pos, int r, int c, int value){
    ...
    for(map<pair<int,int>, unordered_set<int> >::iterator it = unassigned_pos.begin(); it != unassigned_pos.end(); ++it){
        ...
        if(it->first.first == r && it->first.second == c){
            ...
            if(p.first == r && p.second != c){
                if(p.first == 3 && p.second == 2) cout << "!!!1" << endl;
                it->second.erase(value);
            }
            if(p.first != r && p.second == c){
                if(p.first == 3 && p.second == 2) cout << "!!!2" << endl;
                it->second.erase(value);
            }
            int row1 = (r / 3) * 3;
            int row2 = (r / 3) * 3 + 2;
            ...
            int column2 = (c / 3) * 3 + 2;
            if(row1 <= p.first && p.first <= row2 && column1 <= p.second && p.second <= column2 && p.first != r && p.second != c){
                if(p.first == 3 && p.second == 2) cout << "!!!3" << endl;
                it->second.erase(value);
            }
        }
    }
    /*cout << "map: r = " << r << ", c = " << c << endl;
    for(map<pair<int,int>, unordered_set<int> >::iterator it = unassigned_pos.begin(); it != unassigned_pos.end(); ++it){
...
int CSP(vector<vector<int> >& puzzle){
    ...
    cell *root = new cell();
    root->parent = NULL;
    generate_children2(puzzle, unassigned_pos, root, root->children);
    cell *current = root;
    ...
    while(unassigned_total > 0 && steps <= 10000){
        ...
        if(current->children.size() == 0){
            cout << "test1" << endl;
            unassigned_total += back_tracking(current, puzzle, unassigned_pos);
            
        }else if(forward_check(unassigned_pos)){
            cout << "test2" << endl;
            unassigned_total += back_tracking(current, puzzle, unassigned_pos);
        }else{
            cell *prev = current;
            ...
            generate_children2(puzzle, unassigned_pos, current, current->children);
            unassigned_total -= 1;
            steps += 1;
    }
